...
test('is possible to fill in a form and verify error messages (with the help of jest-dom https://testing-library.com/docs/ecosystem-jest-dom)', async () => {
  ...
  await userEvent.type(nameControl, 'Tim');
  await userEvent.clear(scoreControl);
  await userEvent.type(scoreControl, '12');
  fireEvent.blur(scoreControl);
  await userEvent.selectOptions(colorControl, 'G');
  ...
  await userEvent.clear(scoreControl);
  await userEvent.type(scoreControl, '7');
  fireEvent.blur(scoreControl);
